...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/tojbk/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
sed -i '/tojbk/d; /helloworld/d; /passwall/d; /OpenClash/d' "feeds.conf.default"
cat feeds.conf.default|awk '!/^#/'|awk '!/^$/'|awk '!a[$1" "$2]++{print}' >uniq.conf
mv -f uniq.conf feeds.conf.default

# 这里增加了源,要对应的删除/etc/opkg/distfeeds.conf插件源
cat >>"feeds.conf.default" <<-EOF
src-git danshui1 https://github.com/tojbk/openwrt-package.git;${SOURCE}
EOF
...
if [[ `find "${App_path}" -type d -name "zh_Hans" |grep -c "zh_Hans"` -gt '20' ]]; then
  echo "src-git danshui2 https://github.com/tojbk/openwrt-package.git;Theme2" >> "feeds.conf.default"
else
  echo "src-git danshui2 https://github.com/tojbk/openwrt-package.git;Theme1" >> "feeds.conf.default"
fi
...
if [[ `grep -c "CONFIG_PACKAGE_luci-theme-argon=y" ${HOME_PATH}/.config` -eq '1' ]]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/tojbk/openwrt-package/usb/argon/jpg/${pmg}.jpg -o ${HOME_PATH}/files/www/luci-static/argon/background/argon.jpg
  if [[ $? -ne 0 ]]; then
    echo "拉取文件错误,请检测网络"
    exit 1
  fi
...
if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
  ...
  wget -q https://github.com/tojbk/common/releases/download/API/AdGuardHome.api -O AdGuardHome.api
  if [[ $? -ne 0 ]];then
    curl -fsSL https://github.com/tojbk/common/releases/download/API/AdGuardHome.api -o AdGuardHome.api
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/tojbk/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
